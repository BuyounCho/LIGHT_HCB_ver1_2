...
int main() {
    ...
    //  bno rom
    spi_eeprom_write(RID_BNO, (int16_t)5);
    ...
    if(Classify_ValveType()==VALVETYPE_LIGHTDDV) {
        ...
        ValvePosOff_for_FlowrateID[1] = 10.0f;
        ValvePosOff_for_FlowrateID[2] = -10.0f;
        ValvePosOff_for_FlowrateID[3] = 20.0f;
        ValvePosOff_for_FlowrateID[4] = -20.0f;
        ValvePosOff_for_FlowrateID[5] = 30.0f;
        ValvePosOff_for_FlowrateID[6] = -30.0f;
        ValvePosOff_for_FlowrateID[7] = 40.0f;
        ValvePosOff_for_FlowrateID[8] = -40.0f;
        ValvePosOff_for_FlowrateID[9] = 50.0f;
        ValvePosOff_for_FlowrateID[10] = -50.0f;
        ValvePosOff_for_FlowrateID[11] = 60.0f;
        ValvePosOff_for_FlowrateID[12] = -60.0f;
        ValvePosOff_for_FlowrateID[13] = 70.0f;
        ValvePosOff_for_FlowrateID[14] = -70.0f;
        ValvePosOff_for_FlowrateID[15] = 80.0f;
        ValvePosOff_for_FlowrateID[16] = -80.0f;
        ValvePosOff_for_FlowrateID[17] = 100.0f;
        ValvePosOff_for_FlowrateID[18] = -100.0f;
        ValvePosOff_for_FlowrateID[19] = 120.0f;
        ValvePosOff_for_FlowrateID[20] = -120.0f;
        ValvePosOff_for_FlowrateID[21] = 140.0f;
        ValvePosOff_for_FlowrateID[22] = -140.0f;
        ValvePosOff_for_FlowrateID[23] = 160.0f;
        ValvePosOff_for_FlowrateID[24] = -160.0f;
        ValvePosOff_for_FlowrateID[25] = 180.0f;
        ValvePosOff_for_FlowrateID[26] = -180.0f;
        ValvePosOff_for_FlowrateID[27] = 200.0f;
        ValvePosOff_for_FlowrateID[28] = -200.0f;
        ValvePosOff_for_FlowrateID[29] = 240.0f;
        ValvePosOff_for_FlowrateID[30] = -240.0f;
        ValvePosOff_for_FlowrateID[31] = 280.0f;
        ValvePosOff_for_FlowrateID[32] = -280.0f;
        ValvePosOff_for_FlowrateID[33] = 320.0f;
        ValvePosOff_for_FlowrateID[34] = -320.0f;
        ValvePosOff_for_FlowrateID[35] = 360.0f;
        ValvePosOff_for_FlowrateID[36] = -360.0f;
        ValvePosOff_for_FlowrateID[37] = 400.0f;
        ValvePosOff_for_FlowrateID[38] = -400.0f;
        ValvePosOff_for_FlowrateID[39] = 450.0f;
        ValvePosOff_for_FlowrateID[40] = -450.0f;
        ValvePosOff_for_FlowrateID[41] = 500.0f;
        ValvePosOff_for_FlowrateID[42] = -500.0f;
        ValvePosOff_for_FlowrateID[43] = 550.0f;
        ValvePosOff_for_FlowrateID[44] = -550.0f;
        ValvePosOff_for_FlowrateID[45] = 600.0f;
        ValvePosOff_for_FlowrateID[46] = -600.0f;
        ValvePosOff_for_FlowrateID[47] = 675.0f;
        ValvePosOff_for_FlowrateID[48] = -675.0f;
        ValvePosOff_for_FlowrateID[49] = 750.0f;
        ValvePosOff_for_FlowrateID[50] = -750.0f;
        for (int i = 0; i < NUM_FLOWRATE_VS_VALVEPOS_ID; i++) {
            if (i % 2 == 0) {
                // ValvePosOff_for_FlowrateID[i] = -((float)i * 0.5f * 34.0f);
                ValvePosOff_for_FlowrateID_Sorted[(NUM_FLOWRATE_VS_VALVEPOS_ID-1-i)/2] = ValvePosOff_for_FlowrateID[i];
            } else {
                // ValvePosOff_for_FlowrateID[i] = (float)(i+1) * 0.5f * 34.0f;
    ...
    if (TIM2->SR & TIM_SR_UIF) {
        ...
        case MODE_JOINT_CONTROL: {
            ...
            if (Classify_ActuatorType() == ACTUATORTYPE_ROT) { // Rotary Mode
                ...
                float ForceError_DZ = 50.0f; 
                if (force.err < ForceError_DZ && force.err > -ForceError_DZ) {
                    force.err = 0.0f;
                } else if (force.err >= ForceError_DZ) {
                    force.err_int += (force.err-ForceError_DZ) / ((float)TMR_FREQ_5k);
                } else {
                    force.err_int += (force.err+ForceError_DZ) / ((float)TMR_FREQ_5k);
                }
                if(I_GAIN_JOINT_TORQUE*force.err_int>10000.0f) force.err_int = 10000.0f/I_GAIN_JOINT_TORQUE;
                if(I_GAIN_JOINT_TORQUE*force.err_int<-10000.0f) force.err_int = -10000.0f/I_GAIN_JOINT_TORQUE;
        ...
        if (Classify_ValveType()==VALVETYPE_MOOG || Classify_ValveType()==VALVETYPE_KNR) { // Moog Valve or KNR Valve
            ...
            V_out = V_out + 2000.0f * sin(2.0f * PI * 400.0f * ((float)cnt_jitter) * DT_5k); // 4000, 400Hz
            cnt_jitter++;
            if (cnt_jitter == DT_5k)
                cnt_jitter = 0;
        }
...
float Mapping_ValvePos2PWM(float _REF_VALVE_POS) {
    ...
    //     for (int i = 0; i < (NUM_VALVEPOS_VS_PWM_ID-1); i++) {
    //         if (_REF_VALVE_POS <= VALVE_POS_VS_PWM_Sorted[i] && _REF_VALVE_POS >= VALVE_POS_VS_PWM_Sorted[i+1] ) {
    //             _PWM = PWMs_for_ValvePosID_Sorted[i+1] + (PWMs_for_ValvePosID_Sorted[i+1]-PWMs_for_ValvePosID_Sorted[i])*(_REF_VALVE_POS-VALVE_POS_VS_PWM_Sorted[i])/(VALVE_POS_VS_PWM_Sorted[i+1]-VALVE_POS_VS_PWM_Sorted[i]);
    //             _PWM = _PWM * 1000.0f;
    //             return _PWM;
    //         }
    //     }
        ...
        switch(STAGE_VALVEPOS_VS_PWM_ID) {
            ...
            case VALVEPOS_VS_PWM_ID_MAIN:
            {
                ...
                if (CNT4UtilityMode < TMR_FREQ_5k * 1) {
                    // Vout.ref = 3000.0f * sin(2.0f*3.14159f*(float)CNT4UtilityMode*(float)DT_5k * 100.0f);
                    Vout.ref = 0.0f;
...
void UtilFunc_ID_FLOWRATE_VS_VALVEPOS(void)
{
    switch(STAGE_FLOWRATE_VS_VALVEPOS_ID) {
        ...
        case FLOWRATE_VS_VALVEPOS_ID_TERMINATE:
        {
            ...
            if(DIR_VALVE_ENC > 0) {
                if(Qmax<FlowRate_VS_ValvePosOff[1]) {
                ...
                if(Qmin>FlowRate_VS_ValvePosOff[2]) {
                ...
                for (int i = 3; i < UtilityMode_ID_index; i++) {
                    if(i % 2 == 0) { // (-) direction PWM
                        if(Qmin>FlowRate_VS_ValvePosOff[i]) {
                        ...
                        if(Qmax<FlowRate_VS_ValvePosOff[i]) {
                ...
                if(Qmin>FlowRate_VS_ValvePosOff[1]) {
                    FlowRate_VS_ValvePosOff_temp[(UtilityMode_ID_index+1)/2] = FlowRate_VS_ValvePosOff[1];                   
                    Qmin = FlowRate_VS_ValvePosOff[1];   
                } else {
                    FlowRate_VS_ValvePosOff_temp[(UtilityMode_ID_index+1)/2] = Qmin - 0.001f;   
                    Qmin = Qmin - 0.001f;   
                }   

                if(Qmax<FlowRate_VS_ValvePosOff[2]) {
                    FlowRate_VS_ValvePosOff_temp[(UtilityMode_ID_index-3)/2] = FlowRate_VS_ValvePosOff[2];   
                    Qmax = FlowRate_VS_ValvePosOff[2];   
                } else {
                    FlowRate_VS_ValvePosOff_temp[(UtilityMode_ID_index-3)/2] = Qmax + 0.001f;   
                    Qmax = Qmax + 0.001f;   
                }    
 
                for (int i = 3; i < UtilityMode_ID_index; i++) {
                    if(i % 2 == 1) { // (-) direction PWM
                        if(Qmin>FlowRate_VS_ValvePosOff[i]) {
                            FlowRate_VS_ValvePosOff_temp[(UtilityMode_ID_index+i)/2] = FlowRate_VS_ValvePosOff[i];   
                            Qmin = FlowRate_VS_ValvePosOff[i];
                        } else {
                            FlowRate_VS_ValvePosOff_temp[(UtilityMode_ID_index+i)/2] = Qmin - 0.001f;   
                            Qmin = Qmin - 0.001f;   
                        }         
                    } else {
                        if(Qmax<FlowRate_VS_ValvePosOff[i]) {
                            ...
                            FlowRate_VS_ValvePosOff_temp[(UtilityMode_ID_index-i-1)/2] = Qmax + 0.001f;   
                            Qmax = Qmax + 0.001f;   
                        }    
                    }
                }
